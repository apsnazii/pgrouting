...
``pgr_prim`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. code-block:: none
    ...
    pgr_prim(edges_sql, start_vid, end_vid)
...
.. index::
    single: prim(Complete signature)
...
.. code-block:: none
    ...
    pgr_prim(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
