...
``pgr_naziiDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: naziiDijkstra(Minimal Use)
...
.. code-block:: none
    ...
    pgr_naziiDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_naziiDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: naziiDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_naziiDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
